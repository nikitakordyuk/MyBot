...
public class NonCommand {
    ...
    public String nonCommandExecute(Long chatId, String userName, String text) {
        logger.debug(String.format(Utils
                        .convert("Пользователь %s. Начата обработка сообщения \"%s\", не являющегося командой"),
                userName, text));
        ...
        try {
            ...
            logger.debug(String.format("Пользователь %s. Объект настроек из сообщения \"%s\" создан и сохранён",
                    userName, text));
            answer = String.format(Utils
                    .convert("Настройки обновлены. Вы всегда можете их посмотреть с помощью /settings%s"),
            ...
            answer = Utils.convert("Простите, я не понимаю Вас. Похоже, что Вы ввели сообщение, " +
                    "не соответствующее формату, или " +
    ...
    private Settings createSettings(String text) throws IllegalArgumentException {
        ...
        if (text == null) {
            throw new IllegalArgumentException(Utils.convert("Сообщение не является текстом"));
        }
        ...
        if (parameters.length != 3) {
            throw new IllegalArgumentException(String.format(Utils
                            .convert("Не удалось разбить сообщение \"%s\" на 3 составляющих"),
                    text));
        }
    ...
    private void validateSettings(int min, int max, int listCount) {
        if (min == 0 || max == 0 || listCount == 0) {
            throw new IllegalStateException("\uD83D\uDCA9" + Utils
                    .convert("Ни один из параметров не может равняться 0. Поражён " +
                    "Вашей неудачей"));
        }
        if (min > 10000 || max > 10000) {
            throw new IllegalStateException("\uD83D\uDCA9" + Utils
                    .convert("Заданные значения слишком велики. Не забывайте, мы учим " +
    ...
    private String createSettingWarning(Settings settings) {
        return (settings.getPlusMinusUniqueTaskCount() == 0) ?
                String.format("\r\n\r\n\uD83D\uDCA9" + Utils
                        .convert("Для пары чисел %s - %s не существует сложений и вычитаний, " +
